...
$sales_data = mysql_query("SELECT * from tbl_consumbales_header  where company_id = '$company_id' and branch_id = '$branch_id' AND (date_added >= '$start_date' and date_added <= '$end_date') order by date_added DESC");
...
// $sales_data =mysql_query("SELECT * FROM `tbl_dr_header` where dr_date ='2020-09-17' order by dr_date DESC");
	...
	while ($data = mysql_fetch_array($sales_data)) {
        $response["consumbales_header_id"] = $data["consumbales_header_id"];
